          ...
          orderData.map((item, index) => (
            <div key={index} className='py-4 border-t border-b text-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
              <div className='flex items-start gap-6 text-sm'>
                <img className='w-16 sm:w-20 rounded-xl' src={item.image} alt="" />
                <div>
              ...
              <div className='md:w-1/2 flex justify-between'>
                ...
                <button onClick={loadOrderData} className='border px-4 py-2 text-sm rounded-xl border-gray-500 hover:bg-gray-600 hover:text-white cursor-pointer'>Track Order</button>
              </div>
            </div>
          ))
        }
